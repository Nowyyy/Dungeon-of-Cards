...
int deplacement_rectangle_selection(SDL_Rect jouer, SDL_Rect charger, SDL_Rect quitter, SDL_Rect *rect_sel){
	...
	if(event.type == SDL_KEYDOWN){	//touche enfoncée
		if(event.key.keysym.sym == SDLK_DOWN){
			if(rect_sel->h != quitter.h){//on n'est pas sur la dernière option, on peut descendre
				if(rect_sel->h == jouer.h){
					rect_sel->h = charger.h;
				}
				else if(rect_sel->h == charger.h){
					rect_sel->h = quitter.h;
				}
			}
		}
		else if(event.key.keysym.sym == SDLK_UP){
			if(rect_sel->h != jouer.h){//on n'est pas sur la premiere option, on peut monter
				if(rect_sel->h == charger.h){
					rect_sel->h = jouer.h;
				}
				else if(rect_sel->h == quitter.h){
					rect_sel->h = charger.h;
				}
			}
		}
